...
def parse_args():
    ...
    argparser.add_argument("-exe", type=str, default="clang-tidy-12",
                           help="Path to clang-tidy exe")
    argparser.add_argument("-ignore", type=str, default="[.]cxx$|[.]cpp$|[.]cc$",
...
def get_gpu_archs(command):
    archs = []
    for info in command:
        if "--generate-code" not in info:
            continue
        arch_flag = []
        if "compute_" in info:
            arch_flag = info.split("compute_")[1][:2]
        elif "sm_" in info:
            arch_flag = info.split("sm_")[1][:2]
        if arch_flag.isdigit():
            archs.append("--cuda-gpu-arch=sm_%s" % arch_flag)
    return archs
...
def get_tidy_args(cmd, exe):
    ...
    if is_cuda:
        ...
        for info in command:
            if "--generate-code" in info:
                remove_item(command, info)
        ...
        remove_item(command, "--expt-relaxed-constexpr")
