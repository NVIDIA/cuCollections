...
// Its slot is `cuco::pair<stored_key, value_type>` where `stored_key = cuco::pair<int, int>` (8B)
// and `value_type = float` (4B) -- a 12-byte padded slot, which is never single-CAS, so Robin Hood
// cannot support it. No value-type tweak rescues it: an 8-byte composite key forces the slot to
// >= 12 bytes (or 16, which needs 128-bit atomics). Heterogeneous lookup additionally requires the
// hasher to accept the *stored* key type, which Robin Hood needs in order to re-hash residents.
// See stuff_to_raise_in_pr.md and robin_hood_refactor_plan.md.
//
// /**
...
// Stub main so the disabled example still links.
int main() { return 0; }
