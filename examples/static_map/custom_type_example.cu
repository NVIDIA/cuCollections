...
// This example uses custom struct key/value types. Robin Hood requires a single-CAS slot, and its
// erase stores the tombstone age in the payload (`static_cast<size_type>(extract_payload(slot))`),
// which requires an arithmetic payload convertible to size_type -- a custom struct payload cannot
// satisfy either. See stuff_to_raise_in_pr.md and robin_hood_refactor_plan.md.
//
