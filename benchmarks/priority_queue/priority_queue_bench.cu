...
template <typename Key, typename Value, int NumKeys, int NodeSize>
static void BM_insert(::benchmark::State& state)
{
  for (auto _ : state) {
    state.PauseTiming();

    priority_queue<pair<Key, Value>, pair_less<pair<Key, Value>>> pq(NumKeys, NodeSize);
...
BENCHMARK_TEMPLATE(BM_insert, int, int, 128'000'000, 1024)
  ->Unit(benchmark::kMillisecond);

BENCHMARK_TEMPLATE(BM_delete, int, int, 128'000'000, 1024)
  ->Unit(benchmark::kMillisecond);

BENCHMARK_TEMPLATE(BM_insert, int, int, 256'000'000, 1024)
  ->Unit(benchmark::kMillisecond);

BENCHMARK_TEMPLATE(BM_delete, int, int, 256'000'000, 1024)
  ->Unit(benchmark::kMillisecond);

BENCHMARK_TEMPLATE(BM_insert, int, int, 128'000'000, 64)
  ->Unit(benchmark::kMillisecond);

BENCHMARK_TEMPLATE(BM_delete, int, int, 128'000'000, 64)
  ->Unit(benchmark::kMillisecond);

BENCHMARK_TEMPLATE(BM_insert, int, int, 256'000'000, 64)
  ->Unit(benchmark::kMillisecond);

BENCHMARK_TEMPLATE(BM_delete, int, int, 256'000'000, 64)
  ->Unit(benchmark::kMillisecond);
