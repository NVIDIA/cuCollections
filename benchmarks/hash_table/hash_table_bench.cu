...
/*
BENCHMARK_TEMPLATE(BM_cuco_insert_unique_keys, uint32_t, uint32_t)
    ->Unit(benchmark::kMillisecond)
...
/*
BENCHMARK_TEMPLATE(BM_cudf_insert_unique_keys, uint32_t, uint32_t)
    ->Unit(benchmark::kMillisecond)
    ->Apply(generate_size_and_occupancy);
*/
...
/*
BENCHMARK_TEMPLATE(BM_slabhash_insert_unique_keys, uint32_t, uint32_t)
    ->Unit(benchmark::kMillisecond)
    ->Apply(genSizeSlabs);
*/
...
static void BM_slabhash_search_all(::benchmark::State& state) {
  ...
  for(auto _ : state) {
    ...
    searchTime += map.hash_search_bulk(keys, results, numKeys);
