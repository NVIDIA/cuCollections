...
enum class Extent { DYNAMIC, STATIC };
...
struct always_false {
  ...
  __host__ __device__ operator bool() { return b; }
...
__global__ void dynamic_shmem_insert_kernel(std::size_t num_keys,
                                            std::size_t multiplicity,
                                            std::size_t capacity,
                                            always_false pred,
                                            bool* do_not_use)
{
  using Map = typename cuco::static_reduction_map<Op, Key, Value, cuda::thread_scope_block>::
    device_mutable_view<cuco::dynamic_extent>;
...
#pragma nv_diag_suppress static_var_with_dynamic_init
  ...
  // Placeholder predicated store to inject artificial side-effects and keep compiler from
  // discarding the code above
  if (pred) { *do_not_use = result; }
}

template <typename Key, typename Value, typename Op, nvbench::int32_t Capacity>
__global__ void static_shmem_insert_kernel(std::size_t num_keys,
                                           std::size_t multiplicity,
                                           always_false pred,
                                           bool* do_not_use)
{
  using Map = typename cuco::static_reduction_map<Op, Key, Value, cuda::thread_scope_block>::
    device_mutable_view<Capacity>;
...
void nvbench_cuco_static_reduction_map_shmem(nvbench::state& state,
                                             nvbench::type_list<Key,
                                                                Value,
                                                                nvbench::enum_type<Op>,
                                                                nvbench::enum_type<Capacity>,
                                                                nvbench::enum_type<E>>)
{
  ...
  if (num_keys > Capacity) {
    state.skip("Number of input keys exceeds the table's capacity.");
    return;
  }

  state.exec([&](nvbench::launch& launch) {
    if constexpr (E == Extent::STATIC) {
      static_shmem_insert_kernel<Key, Value, OpType, Capacity>
        <<<512, 1024, 0, launch.get_stream()>>>(
          num_keys, multiplicity, always_false{}, (bool*)nullptr);
    } else {
      using slot_type =
        typename cuco::static_reduction_map<OpType, Key, Value, cuda::thread_scope_block>::
          device_mutable_view<>::slot_type;
      dynamic_shmem_insert_kernel<Key, Value, OpType>
        <<<512, 1024, Capacity * sizeof(slot_type), launch.get_stream()>>>(
          num_keys, multiplicity, Capacity, always_false{}, (bool*)nullptr);
    }
  });
}
...
NVBENCH_BENCH_TYPES(nvbench_cuco_static_reduction_map_shmem,
                    NVBENCH_TYPE_AXES(key_type_range,
                                      value_type_range,
                                      op_type_range,
                                      capacity_range,
                                      extent_options))
  .set_name("cuco_static_reduction_map_insert_shmem_extent")
