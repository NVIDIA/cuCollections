...
void nvbench_static_set_contains(
  nvbench::state& state,
  nvbench::type_list<Key, nvbench::enum_type<Dist>, nvbench::enum_type<Multiplicity>>)
{
  ...
  myset.insert(d_keys.begin(), d_keys.end());
  ...
  state.exec(nvbench::exec_tag::sync, [&](nvbench::launch& launch) {
    myset.contains(
      d_keys.begin(), d_keys.end(), thrust::make_discard_iterator(), launch.get_stream());
  });
}
...
NVBENCH_BENCH_TYPES(nvbench_static_set_contains,
                    NVBENCH_TYPE_AXES(key_type,
                                      ...
                                      multiplicity))
  .set_name("staic_set_contains_uniform_multiplicity")
