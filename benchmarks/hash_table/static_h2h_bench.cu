...
template <typename Key, typename Value, dist_type Dist>
static void BM_static_map_find_all(::benchmark::State& state) {
  using map_type = cuco::dynamic_map<Key, Value,
                                     cuda::thread_scope_device,
                                     cuco::legacy_static_map>;
...
template <typename Key, typename Value, dist_type Dist>
static void BM_static_map_find_none(::benchmark::State& state) {
  using map_type = cuco::dynamic_map<Key, Value,
                                     cuda::thread_scope_device,
                                     cuco::legacy_static_map>;
...
static void BM_slabhash_insert_lf(::benchmark::State& state) {
  ...
  for(uint32_t i = 0; i < num_keys; i += batch_size) {
    ...
    std::cout << "batch_idx " << i / batch_size << std::endl;
  }
  */
...
BENCHMARK_TEMPLATE(BM_static_map_find_all, int32_t, int32_t, dist_type::UNIQUE)
  ->Unit(benchmark::kMillisecond)
  ->Apply(gen_size_and_occupancy)
  ->UseManualTime();

BENCHMARK_TEMPLATE(BM_static_map_find_none, int32_t, int32_t, dist_type::UNIQUE)
  ->Unit(benchmark::kMillisecond)
  ->Apply(gen_size_and_occupancy)
  ->UseManualTime();
