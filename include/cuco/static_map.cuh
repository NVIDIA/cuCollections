...
class static_map {
 ...
 public:
   ...
   * @note If the key `*(first + i)` has a matched `element` in the map, copies `element` to
   * `(output_begin + i)`. Else, copies the empty key sentinel.
   *
   * @tparam InputIt Device accessible input iterator
   * @tparam OutputIt Device accessible output iterator assignable from the map's `value_type`
   *
   * @param first Beginning of the sequence of keys
   * @param last End of the sequence of keys
 ...
 private:
  std::unique_ptr<impl_type const> static_map_impl_;
};
}  // namespace experimental
