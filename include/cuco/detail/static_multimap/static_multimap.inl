...
std::size_t static_multimap<Key, Value, Scope, Allocator, ProbeSequence>::pair_count_if(
  InputIt first,
  InputIt last,
  StencilIt stencil,
  Predicate pred,
  ...
  cudaStream_t stream) const
{
  ...
  detail::pair_count_if_n<block_size, cg_size(), is_outer><<<grid_size, block_size, 0, stream>>>(
    first, num_keys, stencil, pred, d_counter_.get(), view, pair_equal);
  CUCO_CUDA_TRY(cudaMemcpyAsync(
    &h_counter, d_counter_.get(), sizeof(atomic_ctr_type), cudaMemcpyDeviceToHost, stream));
  CUCO_CUDA_TRY(cudaStreamSynchronize(stream));
...
template <typename Key,
          ...
          class ProbeSequence>
template <typename InputIt, typename StencilIt, typename Predicate, typename PairEqual>
std::size_t static_multimap<Key, Value, Scope, Allocator, ProbeSequence>::pair_count_outer_if(
  InputIt first,
  InputIt last,
  StencilIt stencil,
  Predicate pred,
...
template <typename InputIt,
          ...
          typename PairEqual>
std::pair<OutputIt1, OutputIt2>
static_multimap<Key, Value, Scope, Allocator, ProbeSequence>::pair_retrieve_if(
  InputIt first,
  InputIt last,
  StencilIt stencil,
  Predicate pred,
  ...
  cudaStream_t stream) const
{
  ...
  detail::pair_retrieve_if_n<block_size, flushing_cg_size, cg_size(), buffer_size, is_outer>
...
template <typename InputIt,
          ...
          typename PairEqual>
std::pair<OutputIt1, OutputIt2>
static_multimap<Key, Value, Scope, Allocator, ProbeSequence>::pair_retrieve_outer_if(
  InputIt first,
  InputIt last,
  StencilIt stencil,
  Predicate pred,
