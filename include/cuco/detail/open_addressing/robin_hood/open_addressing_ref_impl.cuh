...
#endif
    ;
  static_assert(robin_hood_slot_is_single_cas,
...
#pragma nv_diagnostic pop
  ...
  __device__ bool insert(Value value) noexcept
  {
    ...
    while (true) {
      ...
      for (auto& slot_content : bucket_slots) {
        ...
        // is already consumed), so it is folded into this condition.
        if (eq_res == detail::equal_result::AVAILABLE and not this->is_erased(slot_content)) {
          ...
          if (result.state_ == detail::equal_result::UNEQUAL) {
            switch (this->predicate_.template operator()<is_insert::YES>(
              ...
              case detail::equal_result::AVAILABLE: {
                ...
                bool empty_slot = not this->is_erased(bucket_slots[i()]);
                if (empty_slot) {
  ...
  __device__ cuda::std::pair<iterator, bool> insert_and_find(Value value) noexcept
  {
    ...
    while (true) {
      ...
      for (auto i = 0; i < bucket_size; ++i) {
        ...
        if (eq_res == detail::equal_result::AVAILABLE and not this->is_erased(bucket_slots[i])) {
    ...
    while (true) {
      ...
      for (auto& slot_content : bucket_slots) {
        ...
        if (eq_res == detail::equal_result::EQUAL) {
          ...
          value_type erased = this->robin_hood_erased_sentinel(
            slot_content, static_cast<size_type>(*probing_iter + intra_bucket_index));
                                                    ...
                                                    size_type slot_index) const noexcept
  {
    ...
    return robin_hood::probe_distance<bucket_size>(
      probing_scheme_, this->extract_key(slot), slot_index, storage_ref_.extent());
  }
  ...
  robin_hood_erased_sentinel(value_type const& slot, size_type slot_index) const noexcept
  {
    ...
    auto const age = robin_hood::probe_distance<bucket_size>(
      probing_scheme_, this->extract_key(slot), slot_index, storage_ref_.extent());
