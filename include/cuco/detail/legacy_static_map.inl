...
legacy_static_map<Key, Value, Scope>::legacy_static_map(std::size_t capacity,
...
template <typename Key, typename Value, cuda::thread_scope Scope>
legacy_static_map<Key, Value, Scope>::~legacy_static_map()
{
...
void legacy_static_map<Key, Value, Scope>::insert(InputIt first,
                                           InputIt last,
                                           Hash hash,
                                           KeyEqual key_equal)
{
  ...
  *num_successes_ = 0;
  ...
  detail::legacy_insert<block_size, tile_size>
...
void legacy_static_map<Key, Value, Scope>::find(
  InputIt first, InputIt last, OutputIt output_begin, Hash hash, KeyEqual key_equal) noexcept
{
  ...
  detail::legacy_find<block_size, tile_size, Value>
...
template <typename Key, typename Value, cuda::thread_scope Scope>
template <typename InputIt, typename OutputIt, typename Hash, typename KeyEqual>
void legacy_static_map<Key, Value, Scope>::contains(
  InputIt first, InputIt last, OutputIt output_begin, Hash hash, KeyEqual key_equal) noexcept
{
  auto num_keys         = std::distance(first, last);
  auto const block_size = 128;
  ...
  detail::legacy_contains<block_size, tile_size>
    <<<grid_size, block_size>>>(first, last, output_begin, view, hash, key_equal);
  CUCO_CUDA_TRY(cudaDeviceSynchronize());
}
...
__device__ bool legacy_static_map<Key, Value, Scope>::device_mutable_view::insert(
  ...
  while (true) {
    ...
    if (empty) {
      ...
      if (g.thread_rank() == src_lane) {
        ...
        if(old_pair == expected_pair) { status = insert_result::SUCCESS; }
...
template <typename Key, typename Value, cuda::thread_scope Scope>
template <typename Hash, typename KeyEqual>
__device__ typename legacy_static_map<Key, Value, Scope>::device_view::iterator
legacy_static_map<Key, Value, Scope>::device_view::find(Key const& k,
                                                 Hash hash,
                                                 KeyEqual key_equal) noexcept
{
  ...
  while (true) {
    auto const existing_key = current_slot->first;
...
template <typename Key, typename Value, cuda::thread_scope Scope>
template <typename Hash, typename KeyEqual>
__device__ typename legacy_static_map<Key, Value, Scope>::device_view::const_iterator
legacy_static_map<Key, Value, Scope>::device_view::find(Key const& k, Hash hash, KeyEqual key_equal) const
  noexcept
{
  auto current_slot = initial_slot(k, hash);
...
template <typename CG, typename Hash, typename KeyEqual>
__device__ typename legacy_static_map<Key, Value, Scope>::device_view::iterator
legacy_static_map<Key, Value, Scope>::device_view::find(CG g,
                                                 Key const& k,
                                                 Hash hash,
                                                 KeyEqual key_equal) noexcept
{
...
template <typename Key, typename Value, cuda::thread_scope Scope>
template <typename Hash, typename KeyEqual>
__device__ bool legacy_static_map<Key, Value, Scope>::device_view::contains(Key const& k,
                                                                     Hash hash,
                                                                     KeyEqual key_equal) noexcept
{
  auto current_slot = initial_slot(k, hash);
...
template <typename Key, typename Value, cuda::thread_scope Scope>
template <typename CG, typename Hash, typename KeyEqual>
__device__ bool legacy_static_map<Key, Value, Scope>::device_view::contains(CG g,
                                                                     Key const& k,
                                                                     Hash hash,
                                                                     KeyEqual key_equal) noexcept
{
