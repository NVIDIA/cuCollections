...
constexpr cuCollections::size_type UNKNOWN_STRING_LENGTH{-1};
 ...
 */
__host__ __device__ inline cuCollections::size_type bytes_in_utf8_byte(
    BYTE byte) {
  cuCollections::size_type count = 1;
 ...
 * @return Number of bytes.
 */
__device__ inline cuCollections::size_type string_bytes(const char* str) {
  if (!str) return 0;
  cuCollections::size_type bytes = 0;
  while (*str++) ++bytes;
  return bytes;
}
...
__host__ __device__ inline string_view::string_view(const char* data,
                                                    size_type bytes)
...
__device__ inline string_view::const_iterator::const_iterator(
    const string_view& str, size_type pos)
    : cpos{pos}, p{str.data()}, offset{str.byte_offset(pos)} {}

__device__ inline string_view::const_iterator& string_view::const_iterator::
operator++() {
...
__device__ inline string_view::const_iterator string_view::const_iterator::
operator+(string_view::const_iterator::difference_type off) {
...
__device__ inline int string_view::compare(const char* data,
...
__device__ inline size_type string_view::find(const string_view& str,
                                              size_type pos, int count) const {
  return find(str.data(), str.size_bytes(), pos, count);
}

__device__ inline size_type string_view::find(const char* str, size_type bytes,
                                              size_type pos, int count) const {
...
__device__ inline size_type string_view::find(char_utf8 chr, size_type pos,
                                              int count) const {
...
__device__ inline size_type string_view::rfind(const char* str, size_type bytes,
                                               size_type pos, int count) const {
  const char* sptr = data();
  if (!str || !bytes) return -1;
...
__device__ inline size_type string_view::rfind(char_utf8 chr, size_type pos,
                                               int count) const {
  char str[sizeof(char_utf8)];
  size_type chwidth = strings::detail::from_char_utf8(chr, str);
...
__device__ inline string_view string_view::substr(size_type pos,
...
__device__ inline size_type string_view::split(const char* delim, int count,
                                               string_view* strs) const {
  const char* sptr = data();
  size_type sz = size_bytes();
  if (sz == 0) {
  ...
  //
  size_type dchars =
      (bytes ? strings::detail::characters_in_string(delim, bytes) : 1);
  size_type nchars = length();
...
//
__device__ inline size_type string_view::rsplit(const char* delim, int count,
                                                string_view* strs) const {
  const char* sptr = data();
  size_type sz = size_bytes();
  if (sz == 0) {
...
__device__ inline size_type string_view::character_offset(
...
__host__ __device__ inline size_type to_char_utf8(const char* pSrc,
                                                  char_utf8& chr) {
...
__host__ __device__ inline size_type characters_in_string(const char* str,
                                                          size_type bytes) {
