...
__global__ void insert_test_kernel(
  MapRef map_ref, Key key, Value value, bool* inserted)
{
...
TEMPLATE_TEST_CASE_SIG(
  "static_map insert and insert_and_find value_type overloads",
  "",
  ...
  (int64_t, int64_t))
{
  using probing_scheme = cuco::linear_probing<
    1,
  ...
  SECTION("insert_and_find accepts a brace-initialized value_type")
  {
    ...
    insert_and_find_test_kernel<<<1, 1>>>(
      insert_and_find_ref,
      Key{42},
      Value{123},
      inserted.data().get(),
